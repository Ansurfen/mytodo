...
class TaskForm {
  ...
  static List<TaskForm> option(int id) {
    var list = [
      TaskForm(TaskCondType.hand, "task_cond_hand".tr, "task_cond_hand_more".tr,
          onTap: (v) {
        commitTask(CommitTaskRequest(id, TaskCondType.hand.index, ""))
            .then((value) => null);
      })
        ..selected = true,
      TaskForm(TaskCondType.timer, "task_cond_sign_timeout".tr,
          "task_cond_sign_timeout_more".tr),
      TaskForm(TaskCondType.locale, "task_cond_locale".tr,
          "task_cond_locale_more".tr, onTap: (v) {
        RouterProvider.viewMapLocate()?.then((res) {
          commitTask(CommitTaskRequest(id, TaskCondType.locale.index, res))
              .then((res) {
            // for (var form in forms.value) {
      ...
      TaskForm(TaskCondType.file, "task_cond_file_upload".tr,
          "task_cond_file_upload_more".tr),
      TaskForm(TaskCondType.image, "task_cond_image_upload".tr,
          "task_cond_image_upload_more".tr),
      TaskForm(TaskCondType.content, "task_cond_text_content".tr,
          "task_cond_text_content_more".tr),
      TaskForm(
          TaskCondType.qr, "task_cond_qr_scan".tr, "task_cond_qr_scan_more".tr),
    ];
    return list;
  }
}
